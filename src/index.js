...
function createToyCard(json) {
  ...
  json.forEach(toy => {
    ...
    uniqueCard = toyView.appendChild(div)
    addInfoToCard(toy)
  })
}
...
function addInfoToCard(toy) {
  ...
  button.id = `${toy.id}`
  uniqueCard.appendChild(h2).textContent = toy.name
  uniqueCard.appendChild(img).src = toy.image
  uniqueCard.appendChild(p).textContent = `${toy.likes} Likes`
  uniqueCard.appendChild(button).textContent = `Like`
